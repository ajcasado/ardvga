...
void setup() {
  mivga.begin(24,32,1); //107599 80939 56759 131112 106340 100155
  //mivga.setSkipLine();
  //mivga.setMode_720();
  mivga.tone (NOTE_A4,100);
  mivga.delay(1500);
...
void loop() {
  ...
  mivga.loadzxscr(tmpBitmaps, tmpAttributes,0);
  mivga.delay(5000);
...
//  mivga.setNoSkipLine();
  ...
  mivga.cls();
  for (uint8_t i = 0; i < mivga.getVChars(); i++) mivga.scrolltext(1);

  uint16_t a0 = analogRead(0);
  float Vcc = (float)a0;
  Vcc = Vcc * 4.8876;
  uint16_t mVcc = (int)Vcc;
  for (uint8_t r=0;r<5;r++){
    uint32_t cic = mivga.countLines();
    sprintf_P(buf, PSTR("Hola mundo!\0"));
    mivga.setCursor(12,12);
    mivga.print(buf);
...
//mivga.loadzxscr(spectrumBitmaps, spectrumAttributes);
