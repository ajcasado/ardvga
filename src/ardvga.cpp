...
volatile uint16_t ardvga::scanLine = 1;
...
void ardvga::begin(uint8_t height, uint8_t width, uint8_t doSplash /*añadir modo y hT como argumntos?*/)
{
  ...
  if (doSplash)
  {
    ...
    print(initMessage);
    sprintf_P(initMessage, PSTR("|Version 0.20|\n\0"));
    print(initMessage);
    sprintf_P(initMessage, PSTR("|AJCA<-->2020|\n\0"));
    print(initMessage);
    sprintf_P(initMessage, PSTR("==============\n\0"));
    print(initMessage);
    ...
    print(initMessage);
    for (uint8_t i = inkBlue; i < inkWhite; i++)
      setattr(0, i - 1, Paper, i, PaperBright, InkBright);
    InkBright = brightInk;
    for (uint8_t i = inkBlue; i <= inkWhite; i++)
      setattr(0, i + 6, Paper, i, PaperBright, InkBright);
  }
  //tone (NOTE_A4,1000);
}
void ardvga::end() {}
...
void ardvga::toggleMode(){
  switch (mode) {
    case _720:
      mode = _640;
      break;
    case _640://volatile uint8_t ardvga::doLine = 0;
...
bool ardvga::setResolution(uint8_t height, uint8_t width)
{
  ...
  if (bitmask = (uint8_t *)malloc(((height * 8) * width) * sizeof(uint8_t)))
    if (attributes = (uint8_t *)malloc((height * width) * sizeof(uint8_t)))
    {
      ...
      vFrontPorch = (((verticalChars * 8) + 9) + 8) * V_PIXEL_SCALE; //esto cambia para 328p
      bitmaskBck = bitmask;
...
{
  if ((y < 0) || ((y > verticalCharsBck - 1)))
    return 1;
  if ((height < 0) || ((height > (verticalCharsBck - y))))
    return 1;
...
ISR(TIMER2_OVF_vect)
{
  ...
  {
  case 1:
    vsyncon(); //V_SYNC_PORT |= VSYNC_PIN_UP_MASK;
    break;
  case 3:
    vsyncoff(); //V_SYNC_PORT &= VSYNC_PIN_DOWN_MASK;
    if (ardvga::sndDur)
      ardvga::sndDur--;
    else
    {
  ...
  if (ardvga::sndDur)
  {
    if (ardvga::hLine++ > ardvga::sndFreq)
      ardvga::hLine = 0;
    else if (ardvga::hLine > (ardvga::sndFreq / 2)) // (4/5) es el volumen
      soundoff();
    else
      soundon();
...
ISR(TIMER2_COMPB_vect)
{
  if (ardvga::doLine)
  {
#ifdef EFFECT_SCANLINE
    if (ardvga::scanLine & 1)
    {
#endif
...
{ //
  ...
  while (k < aMemSize)
  {
    ...
    *(attributes + k) = paper | ink | bright;
    ...
    k++;
    while (i < 8)
    { // hay que cambiar 8 por el valor de alto del bloque unitario de atributos
...
{
  if ((column < 0) || (column > horizontalChars - 1) || (line < 0) || (line > verticalChars - 1))
    return 1;
...
void ardvga::setCRTCTRL(uint8_t ht)
{
  ...
  TCCR2A = (1 << COM2B1) | (1 << WGM20) | (1 << WGM21);
  TCCR2B = (1 << WGM22) | (1 << CS21);
  OCR2A = ht;                 //HT-1;//(P_8=107)(P_32=27)(P_64=13)
  OCR2B = ((ht + 1) / 8) - 1; //HPT-1;//(P_8=13)(P_32=3)(P_64=2)
...
{
  if (i > ardvga::verticalChars)
    i = ardvga::verticalChars;
  scrollbu(8 * i);
  scrollau(i);
}
bool ardvga::putStr(char str[], size_t strSize, uint8_t line, uint8_t column, uint8_t paper, uint8_t ink, uint8_t paperbright, uint8_t inkbright)
{ //hacer una igual para posicionar al pixel, hacer un tipo estructura para atributos
...
bool ardvga::putChar(uint8_t c, uint8_t line, uint8_t column)
{ //hacerla para fuente generica y para alinear a pixel o a atributo
  if ((column < 0) || (column > horizontalChars - 1) || (line < 0) || (line > verticalChars - 1))
    return 1;
  uint8_t aux; // k;
...
{
  if ((x < 0) || (x > horizontalChars - 1) || (y < 0) || (y > verticalChars - 1))
    return 1;
  xPos = x;
  yPos = y;
}
...
{
  if (ardvga::mode == _640)
    return (ardvga::hFreq(ht) / 525);
  if (ardvga::mode == _720)
    return (ardvga::hFreq(ht) / 449);
}
...
{
  if ((x < 0) || (x > horizontalTop) || (y < 0) || (y > verticalTop))
    return 1;
...
{
  if ((frequency > (hFreq(hT))) || (frequency < ((hFreq(hT) / 255))))
    return; //implementar maxSFreq y minSfreqpúblicas y calcularlas en setResolution
